...
QBCore.Functions.CreateCallback("okokBanking:IsIBanUsed", function(source, cb, iban)
	...
	exports['ghmattimysql']:execute('SELECT * FROM players WHERE iban = @iban', {
		...
		if db ~= nil then
			...
			exports['ghmattimysql']:execute('SELECT * FROM societies WHERE iban = @iban', {
				['@iban'] = iban
			}, function(result2)
				local db2 = result2[1]
...
QBCore.Functions.CreateCallback("okokBanking:SocietyInfo", function(source, cb, society)
	exports['ghmattimysql']:execute('SELECT * FROM societies WHERE society = @society', {
		['@society'] = society
	}, function(result)
		local db = result[1]
		cb(db)
...
AddEventHandler("okokBanking:CreateSocietyAccount", function(society, society_name, value, iban)
	exports['ghmattimysql']:execute('INSERT INTO societies (society, society_name, value, iban) VALUES (@society, @society_name, @value, @iban)', {
		['@society'] = society,
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		exports['ghmattimysql']:execute('UPDATE societies SET value = value + @value WHERE society = @society AND society_name = @society_name', {
			['@value'] = amount,
			['@society'] = society,
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	exports['ghmattimysql']:execute('UPDATE societies SET is_withdrawing = 1 WHERE society = @society AND society_name = @society_name', {
		['@value'] = amount,
		['@society'] = society,
		['@society_name'] = societyName,
	}, function(changed)
	...
	exports['ghmattimysql']:execute('UPDATE societies SET is_withdrawing = 0 WHERE society = @society AND society_name = @society_name', {
		['@value'] = amount,
		['@society'] = society,
		['@society_name'] = societyName,
	}, function(changed)
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.PlayerData.money.bank
		...
		if amount <= playerMoney then
			exports['ghmattimysql']:execute('UPDATE societies SET value = value + @value WHERE iban = @iban', {
				['@value'] = amount,
				['@iban'] = ibanNumber
			}, function(changed)
			end)
...
AddEventHandler("okokBanking:AddDepositTransaction", function(amount, source_)
	...
	exports['ghmattimysql']:execute('INSERT INTO player_transactions (receiver_identifier, receiver_name, sender_identifier, sender_name, date, value, type) VALUES (@receiver_identifier, @receiver_name, @sender_identifier, @sender_name, CURRENT_TIMESTAMP(), @value, @type)', {
